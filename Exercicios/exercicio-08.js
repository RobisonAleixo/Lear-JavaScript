...
function estatisticas (array) {
    let bateuSeuProprioRecorde = 0;
    let maiorPontuacao = array[0];
    let piorPontuacao = array[0];
    let piorJogo = 0;

    for(let i = 0; i <= array.length; i++) {
        if (maiorPontuacao < array[i]) {
            maiorPontuacao = array[i];
            bateuSeuProprioRecorde++;
        }
        if (array[i] < piorPontuacao) {
            piorPontuacao = array[i];
            piorJogo = i+1;
        }
    }

    return [bateuSeuProprioRecorde, piorJogo];
}

console.log(estatisticas(pontuacoes));